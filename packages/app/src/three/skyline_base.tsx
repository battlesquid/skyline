...
import { BoxGeometry, type BufferGeometry, type Group, type Mesh, MeshStandardMaterial } from "three";
...
export function SkylineBase(props: SkylineBaseProps) {
    ...
    const [geometry, setGeometry] = useState<BufferGeometry>(new BoxGeometry(0, 0, 0));
    const [rot, setRot] = useState(0);

    useEffect(() => {
        const width = parameters.computed.modelLength + parameters.computed.paddingWidth;
        const length = parameters.computed.modelWidth * years.length + parameters.computed.paddingWidth;
        ...
        <group>
            <group
                ...
                position={[
                    ...
                    (parameters.computed.modelWidth * years.length) / 2 + parameters.inputs.padding + LOGO_DEPTH_OFFSET
                ]}
            />
            <Text3D
                ref={nameRef}
                ...
                position={[
                    ...
                    (parameters.computed.modelWidth * props.years.length) / 2 + parameters.inputs.padding + TEXT_DEPTH_OFFSET,
                ]}
            ...
            <Text3D
                ...
                position={[
                    ...
                    -parameters.computed.platformMidpoint - 0.5,
                    (parameters.computed.modelWidth * props.years.length) / 2 + parameters.computed.paddingWidth / 2 + TEXT_DEPTH_OFFSET,
                ]}
                height={parameters.inputs.textDepth}
                size={parameters.computed.textSize}
            >
            ...
            <mesh onPointerEnter={(e) => e.stopPropagation()} castShadow receiveShadow geometry={geometry} position={[0, -parameters.computed.platformMidpoint, 0]}>
