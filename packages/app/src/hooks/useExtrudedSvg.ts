...
export const useExtrudedSvg = ({
	...
	const [svgBoundingBox, setSvgBoundingBox] = useState<Dimensions>({
		width: 0,
		height: 0,
	});
	...
	useEffect(() => {
		...
		geometries.sort(
			(g1, g2) =>
				getBoundingBoxVolume(g2.boundingBox) -
				getBoundingBoxVolume(g1.boundingBox),
		);
	...
	useEffect(() => {
		if (
			!isNullish(ref) &&
			!isNullish(ref.current) &&
			!isNullish(onGroupReady)
		) {
			ref.current.clear();
