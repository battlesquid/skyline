...
const ContributionBackground: React.FC<ContributionBackgroundProps> = ({
    ...
    useEffect(() => {
        ...
        const drawGrid = () => {
            ...
            const startX = -((offsetX % totalCellSize) + totalCellSize) % totalCellSize;
            const startY = -((offsetY % totalCellSize) + totalCellSize) % totalCellSize;
        ...
        const updateAnimation = (timestamp: number) => {
            ...
            switch (direction) {
                case "right":
                    gridOffset.current.x = mod(gridOffset.current.x + effectiveSpeed, patternPeriodX);
                    break;
                case "left":
                    gridOffset.current.x = mod(gridOffset.current.x - effectiveSpeed, patternPeriodX);
                    break;
                case "up":
                    gridOffset.current.y = mod(gridOffset.current.y - effectiveSpeed, patternPeriodY);
                    break;
                case "down":
                    gridOffset.current.y = mod(gridOffset.current.y + effectiveSpeed, patternPeriodY);
                    break;
    ...
    }, [direction, speed, borderColor, hoverFillColor, squareSize, gapSize, randomColors, colorPeriodX, colorPeriodY]);
